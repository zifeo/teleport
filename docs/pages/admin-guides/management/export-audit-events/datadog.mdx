...
<Admonition title="Testing locally?">

Create a local bridge network where you will run the Event Handler and Fluentd,
then generate certificates for the Event Handler, signed for the hostname we'll
provide the Fluentd container on the network:

```code
$ docker network create fluentd-test
$ docker run --network fluentd-test -v `pwd`:/opt/teleport-plugin -w /opt/teleport-plugin public.ecr.aws/gravitational/teleport-plugin-event-handler:16.2.0 configure --dns-names=fluentd .
```
...
1. Create a local Docker network and start a Fluentd container within it:

   ```code
   $ docker run -u $(id -u root):$(id -g root) --hostname fluentd --network fluentd-test -v $(pwd):/keys -v \
   $(pwd)/fluent.conf:/fluentd/etc/fluent.conf --entrypoint=/bin/sh -i --tty  fluent/fluentd:edge
   ```

1. In the container shell, run the following to install the Datadog plugin and
   start Fluentd:
...
Later, you will create a local bridge network and connect the Event Handler to
Fluentd.
...
1. In the Event Handler configuration file, `teleport-event-handler.toml`,
   change the `florward.fluentd.url` and `forward.fluentd.session-url` values
   to, respectively, `https://fluentd:8888/test.log` and
   `https://fluentd:8888/session`.

1. Navigate to the directory where you ran the `event-handler configure` command
   earlier and execute the following command:

   ```code
   $ docker run --network fluentd-test -v `pwd`:/opt/teleport-plugin -w /opt/teleport-plugin public.ecr.aws/gravitational/teleport-plugin-event-handler:(=teleport.version=) start --config=teleport-event-handler.toml 
   ```
