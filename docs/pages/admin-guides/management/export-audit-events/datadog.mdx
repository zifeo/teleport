...
Since the Datadog Agent can only receive logs over a network as JSON-encoded
bytes over a [TCP or UDP
...
1. Visit Datadog and generate an API key for Fluentd. From **Organization
   Settings** -> Access -> **API Keys**, click on **+ New Key**:
...
1. Edit your configuration as follows:
   ...
   - Adjust the `host` value to match your Datadog site. See their [Log
     Collection and
