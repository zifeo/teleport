...
func (f *processState) update(event Event) {
	...
	if !ok {
		f.process.logger.Error("Event broadcast without component name, this is a bug!", "event", event.Name)
		return
	}
	...
	case TeleportDegradedEvent:
		...
		f.process.logger.Info("Detected Teleport component is running in a degraded state.", "component", component)
	...
	case TeleportOKEvent:
		...
		case stateStarting:
			s.state = stateOK
			f.process.logger.Debug("Teleport component has started.", "component", component)
		case stateDegraded:
			s.state = stateRecovering
			s.recoveryTime = f.process.Clock.Now()
			f.process.logger.Info("Teleport component is recovering from a degraded state.", "component", component)
		case stateRecovering:
			if f.process.Clock.Since(s.recoveryTime) > defaults.HeartbeatCheckPeriod*2 {
				s.state = stateOK
				f.process.logger.Info("Teleport component has recovered from a degraded state.", "component", component)
			}
		}
	}
}
