...
// GetCurrentSuiteFromAuthPreference returns a [CurrentSuiteGetter] that fetches
// the signature algorithm suite currently configured in the cluster via the
// current auth preference.
func GetCurrentSuiteFromAuthPreference(authPrefGetter AuthPreferenceGetter) CurrentSuiteGetter {
	return CurrentSuiteGetter(func(ctx context.Context) (types.SignatureAlgorithmSuite, error) {
...
// CurrentSuiteGetter is a function type that retrieves the current signature
// algorithm suite configured in the cluster.
type CurrentSuiteGetter func(context.Context) (types.SignatureAlgorithmSuite, error)

func getSignatureAlgorithmSuite(ctx context.Context, currentSuiteGetter CurrentSuiteGetter) (types.SignatureAlgorithmSuite, error) {
	suite, err := currentSuiteGetter(ctx)
	if err != nil {
		return types.SignatureAlgorithmSuite_SIGNATURE_ALGORITHM_SUITE_UNSPECIFIED, trace.Wrap(err)
	}
...
func AlgorithmForKey(ctx context.Context, currentSuiteGetter CurrentSuiteGetter, purpose KeyPurpose) (Algorithm, error) {
	suite, err := getSignatureAlgorithmSuite(ctx, currentSuiteGetter)
	if err != nil {
...
func GenerateUserSSHAndTLSKey(ctx context.Context, currentSuiteGetter CurrentSuiteGetter) (sshKey, tlsKey crypto.Signer, err error) {
	currentSuite, err := getSignatureAlgorithmSuite(ctx, currentSuiteGetter)
	if err != nil {
...
func GenerateKey(ctx context.Context, currentSuiteGetter CurrentSuiteGetter, purpose KeyPurpose) (crypto.Signer, error) {
	alg, err := AlgorithmForKey(ctx, currentSuiteGetter, purpose)
	if err != nil {
