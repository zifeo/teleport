...
type Server struct {
	...
	*logrus.Entry
	...
	proxyMode        bool
	proxyTun         reversetunnelapi.Tunnel
...
func SetProxyMode(peerAddr string, tsrv reversetunnelapi.Tunnel, ap auth.ReadProxyAccessPoint, router *proxy.Router) ServerOption {
	return func(s *Server) error {
...
func (s *Server) tunnelWithAccessChecker(ctx *srv.ServerContext) (reversetunnelapi.Tunnel, error) {
	clusterName, err := s.GetAccessPoint().GetClusterName()
	if err != nil {
		return nil, trace.Wrap(err)
	}

	return reversetunnelapi.NewTunnelWithRoles(s.proxyTun, clusterName.GetName(), ctx.Identity.AccessChecker, s.proxyAccessPoint), nil
}
