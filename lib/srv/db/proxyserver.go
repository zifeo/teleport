...
type ProxyServerConfig struct {
	...
	Tunnel reversetunnelapi.Server
...
func (s *ProxyServer) Connect(ctx context.Context, proxyCtx *common.ProxyContext, clientSrcAddr, clientDstAddr net.Addr) (net.Conn, error) {
	...
	for _, server := range getShuffleFunc()(proxyCtx.Servers) {
		...
		serviceConn, err := proxyCtx.Cluster.Dial(reversetunnelapi.DialParams{
			From:                  clientSrcAddr,
...
func isReverseTunnelDownError(err error) bool {
	return trace.IsConnectionProblem(err) ||
		strings.Contains(err.Error(), reversetunnelapi.NoDatabaseTunnel)
}
...
func (s *ProxyServer) getDatabaseServers(ctx context.Context, identity tlsca.Identity) (reversetunnelapi.RemoteSite, []types.DatabaseServer, error) {
