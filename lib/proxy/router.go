...
type SiteGetter interface {
	...
	GetSite(clusterName string) (reversetunnelapi.RemoteSite, error)
}
...
type Router struct {
	...
	localSite      reversetunnelapi.RemoteSite
	siteGetter     SiteGetter
...
func (r *Router) DialHost(ctx context.Context, clientSrcAddr, clientDstAddr net.Addr, host, port, clusterName string, accessChecker services.AccessChecker, agentGetter teleagent.Getter, signer agentless.SignerCreator) (_ net.Conn, teleportVersion string, err error) {
	...
	conn, err := site.Dial(reversetunnelapi.DialParams{
		From:                  clientSrcAddr,
...
func (r *Router) getRemoteCluster(ctx context.Context, clusterName string, checker services.AccessChecker) (reversetunnelapi.RemoteSite, error) {
...
// for a reversetunnelapi.RemoteSite. It makes testing easier.
type site interface {
...
func (r *Router) DialSite(ctx context.Context, clusterName string, clientSrcAddr, clientDstAddr net.Addr) (net.Conn, error) {
	...
	if clusterName == r.clusterName {
		conn, err := r.localSite.DialAuthServer(reversetunnelapi.DialParams{From: clientSrcAddr, OriginalClientDstAddr: clientDstAddr})
	...
	conn, err := site.DialAuthServer(reversetunnelapi.DialParams{From: clientSrcAddr, OriginalClientDstAddr: clientDstAddr})
	if err != nil {
		return nil, trace.Wrap(err)
	}
