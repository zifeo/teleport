...
type transportConfig struct {
	proxyClient  reversetunnelapi.Tunnel
...
// tunnel subsystem.
func dialAppServer(ctx context.Context, proxyClient reversetunnelapi.Tunnel, clusterName string, server types.AppServer) (net.Conn, error) {
	...
	conn, err := clusterClient.Dial(reversetunnelapi.DialParams{
		From:                  from,
...
func isReverseTunnelDownError(err error) bool {
	return trace.IsConnectionProblem(err) ||
		strings.Contains(err.Error(), reversetunnelapi.NoApplicationTunnel)
}
