...
func proxyWebsocketConn(ws *websocket.Conn, wds net.Conn, wdsVersion string) error {
	profileWdsBandwidth := true // Note to reviewers: this variable should always be false in production.
	var start time.Time
	...
	close := func() {
		...
		if profileWdsBandwidth {
			...
			log.Debugf("TDP websocket closed. Read %v bytes from wds, wrote %v bytes to wds, in %v. Bytes read/sec: %v, bytes written/sec: %v, Total bandwidth over wds: %v",
				bytesReadFromWds,
				bytesWrittenToWds,
				duration,
				bytesReadPerSec,
