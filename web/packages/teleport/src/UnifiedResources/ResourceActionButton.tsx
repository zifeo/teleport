...
export const ResourceActionButton = ({ resource }: Props) => {
  switch (resource.kind) {
    ...
    case 'kube_cluster':
      ...
      return <KubeExecConnect kube={resource} />;
...
const KubeExecConnect = ({ kube }: { kube: Kube }) => {
  const { clusterId } = useStickyClusterId();

  const startKubeExecSession = (namespace: string, pod: string) => {
  ...
  return (
    <MenuLogin
      ...
      alignButtonWidthToMenu
      getLoginItems={() => {
        // don't populate items for now
        // (user can type NAMESPACE/POD)
        return [];
      }}
      onSelect={(e, login) => {
        const namespacePod = login.split('/');
        if (namespacePod.length == 2) {
          startKubeExecSession(namespacePod[0], namespacePod[1]);
        }
      }}
      transformOrigin={{
        vertical: 'top',
